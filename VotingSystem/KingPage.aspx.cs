...
{
    public partial class KingPage : System.Web.UI.Page
    {
        ...
        {
            string vote = Session["vote_btn"] as String;
            string Success = Session["Success"] as String;
            if (vote == "click")
            {
                KingVote.Visible = false;
                Label6.Text = Success;
            }
            conn.Open();
        ...
        protected void KingVote_Click(object sender, EventArgs e)
        {
            ...
            int count=0;
            string query = "select KingResult from Participant where Name = '" + name + "' ";

            SqlCommand cmd = new SqlCommand(query, conn);
            SqlDataReader dr = cmd.ExecuteReader();
            if (dr.Read())
            {
                string result = dr["KingResult"].ToString();
    ...
    Session["vote_btn"] = "click";
    Session["Success"] = "You Voted " + name + " as a King.";
    Label6.Text = "You Voted " + name + " as a King.";
    KingVote.Visible = false;
        }
    }
}